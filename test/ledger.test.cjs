...
test('30桁・手計算の既知値・金額とIDの検証',()=>{assert.equal(C.hash('00347'+'00052'+'00000'.repeat(4)),'1514');assert.equal(C.hash('00000'.repeat(5)+'00001'),'0011');assert.equal(C.hash('01234'+'00000'.repeat(5),1),'5227');assert.equal(C.units('12.4'),124);for(const x of ['-1','1.22','NaN','100000'])assert.throws(()=>C.units(x));assert.throws(()=>C.id('00000000'));assert.throws(()=>C.id(12345678));});
...
test('複数課題の全Nonceを独立計算し1/3・1/4の最適ターゲットを確認',()=>{
  for(const denominator of [3,4])for(const prevHash of ['0000','4321','9999']) {
    const b={from:alice,to:bob,amount:123,prevHash};const result=C.calibrate(b,denominator);
    const histogram=Array(10000).fill(0);let count=0;
    for(let n=0;n<10000;n++) {
      const chunks=C.payload(b,String(n).padStart(4,'0')).match(/.{5}/g).map(Number);
      const [A,B,Cc,D,E,F]=chunks, x=(A+Cc+E+F)%1000, y=(B+D+E+F)%90+10;
...
test('ターゲット境界以上を拒否し、内部の正解一覧は課題に含めない',t=>{
  const l=new Ledger(setup(t));l.prepare({type:'reward',miner:alice});const b=l.pending;
  for(let n=0;n<10000;n++){const nonce=String(n).padStart(4,'0'),hash=C.hash(C.payload(b,nonce));if(Number(hash)>=b.difficulty){assert.throws(()=>l.submit({token:b.token,nonce,hash}),/判定上限/);break;}}
...
test('旧台帳を保持して新方式へ移行、混在台帳を再起動で復元',t=>{
  ...
  assert.ok(fs.readFileSync(file,'utf8').startsWith(prefix));const restored=new Ledger(file);assert.equal(restored.s.bank[alice],34);assert.deepEqual(restored.s.blocks.map(b=>b.version),[1,1,2]);
});
