...
$('settings').elements.difficulty.value=s.config.difficulty;$('settings').elements.reward.value=coin(s.config.reward);
$('empty').hidden=!!s.pending;$('task').hidden=!s.pending;
$('prepare').querySelector('button').disabled=!!s.pending||s.failed;
if(s.pending){const b=s.pending;const data=b.from+b.to+String(b.amount).padStart(6,'0')+b.prevHash+'????';$('digits').textContent=data.match(/.{5}/g).join(' ');$('taskinfo').textContent=`課題 #${b.index} / 採掘者 ${b.miner} / ${b.type==='reward'?'採掘のみ':b.from+' → '+b.to+' / '+coin(b.amount)+' Coin'}`;$('condition').textContent=`成功条件: Hash < ${b.difficulty} ｜受取: 報酬 ${coin(b.reward)} + 手数料 ${coin(b.fee)} Coin`;}
...
if(name==='settings'){f.difficulty=Number(f.difficulty);f.reward=Math.round(Number(f.reward)*10);}
